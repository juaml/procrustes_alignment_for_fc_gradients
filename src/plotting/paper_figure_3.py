...
def main():
    ...
    path = (
        ...
        / "results"
        / "fd_correlations"
        / "hcp_ya_Schaefer400_correlations_with_FD.csv"
    )
    ...
    with plt.style.context("default.mplstyle"):
        ...
        for index, (row, col) in enumerate(product(range(rows), range(cols))):
            ...
            # sns.kdeplot(
            #    data=subset, x="correlation", color="k", ax=plot, stat="count"
            # )
            ...
            mean = subset["Pearson's r"].mean().round(2)
            sd = subset["Pearson's r"].std().round(2)
            ax.annotate(
        ...
        outname = outpath / "paper" / "paper_figure_3"
